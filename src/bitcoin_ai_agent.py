...
def create_transaction_manual(
    ...
    network: str = "testnet",
    change_index: int = 0,
    tx_opts: Optional[Dict] = None,        # NOVETAT: knobs transaccionals sense política per defecte
    psbt_extras: Optional[Dict] = None,    # NOVETAT: claus PSBT addicionals (global/inputs/outputs/proprietary)
) -> Dict:
    """
    Crea una PSBT triant manualment UTXOs (txid:vout) sense aplicar decisions de privacitat per defecte.
    - tx_opts (opc.): qualsevol d'aquestes claus si es volen forçar:
        rbf, locktime_override, shuffle_inputs, shuffle_outputs, avoid_change, min_change_sats,
        include_global_xpub, include_keypaths, prefer_legacy_witness_utxo, shuffle_seed
      Si no es passen, NO s'imposa res.
    - psbt_extras (opc.): diccionari per injectar claus PSBT:
        {
          "global": {
            "version": 0|2, "fallback_locktime": <int>, "tx_modifiable": <0..255>,
            "proprietary": [{"prefix_hex":..,"subtype":..,"keydata_hex":..,"value_hex":..}, ...]
          },
          "inputs": [
            {
              "sighash_type": <int>, "redeem_script_hex": "...", "witness_script_hex": "...",
              "por_commitment_hex": "...",
              "ripemd160_preimage_hex": "...", "sha256_preimage_hex": "...",
              "hash160_preimage_hex": "...", "hash256_preimage_hex": "...",
              "bip32_derivations": [
                {"pubkey_hex":"...", "fingerprint_hex":"...", "path_indices":[..,..]}
              ]
            }, ...
          ],
          "outputs": [
    ...
    """
    try:
        # Validació/normalització adreça de destí
        ...
        # Llença si l’adreça és invàlida
        PSBTCreator(network=network)._decode_address(recipient_address)

        # Llistar UTXOs i seleccionar els demanats
        ...
        if isinstance(tx_opts, dict):
            for k, v in tx_opts.items():
                if k in allowed_keys:
                    kwargs[k] = v
        if psbt_extras is not None:
            kwargs["psbt_extras"] = psbt_extras

        manual_list = selected if selected else None
        ...
        build = create_transaction_psbt(
            ...
            network=network,
            fee_rate=fee_rate,
            manual_selected_utxos=manual_list,
            **kwargs,
        )

        if build.get("success"):
            build["selection_mode"] = "manual" if manual_list else "auto_fallback"
            build["requested_utxos"] = utxo_ids
            if manual_list:
                build["used_utxos"] = [f"{u.get('txid')}:{u.get('vout')}" for u in manual_list]
            if missing:
                build["missing"] = list(missing)
            if tx_opts:
                build["applied_tx_opts"] = {k: v for k, v in kwargs.items() if k in allowed_keys}
            if psbt_extras is not None:
